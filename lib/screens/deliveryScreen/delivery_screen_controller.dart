...
class DeliveryScreenController extends GetxController {
  ...
  Future<void> searchFood({String? dishName}) async {
    final query = (dishName ?? searchController.text).trim();
    print('searchFood Input: $query');
    if (query.isEmpty) {
      foodItems.clear();
      searchLoading.value = false;
      return;
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse(
          ApiServices.searchFoodData
              .replaceAll('{outletId}', outlateDetails['id'].toString())
              .replaceAll('{dishName}', Uri.encodeComponent(query)),
        ),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': '${storage.read(userToken)}',
      ...
      print('searchFood Response status: ${response.statusCode}');
      print('searchFood Response body: ${response.body}');
      var data = jsonDecode(response.body);
      if (response.statusCode == 200 && data["success"] == true) {
        var items = data["data"] is Map
            ? (data["data"]["items"] ?? [])
